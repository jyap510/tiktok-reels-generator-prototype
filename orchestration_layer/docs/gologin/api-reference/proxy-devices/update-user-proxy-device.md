...
````yaml https://docs-download.gologin.com/openapi.json patch /proxy-devices/{deviceId}
openapi: 3.0.0
info:
  title: GoLogin
  description: ''
...
paths:
  /proxy-devices/{deviceId}:
    patch:
      tags:
        - Proxy Devices
      summary: Update user proxy device
      operationId: ProxyDevicesController_updateDevice
      parameters:
        - name: deviceId
          required: true
          in: path
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/UpdateProxyDeviceValidation'
      responses:
        '200':
          description: User proxy device updated successfully
          content:
            application/json:
              schema:
