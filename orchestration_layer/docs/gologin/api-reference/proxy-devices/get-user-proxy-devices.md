...
````yaml https://docs-download.gologin.com/openapi.json get /proxy-devices
openapi: 3.0.0
info:
  title: GoLogin
  description: ''
...
paths:
  /proxy-devices:
    get:
      tags:
        - Proxy Devices
      summary: Get user proxy devices
      operationId: ProxyDevicesController_getDevices
      parameters:
        - name: country
          required: false
          in: query
          description: Filter by country code (2-letter) or country name
          schema:
            type: string
        - name: smsStatus
          required: false
      ...
      responses:
        '200':
          description: List of user proxy devices retrieved successfully
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/ProxyDeviceListResponseDto'
      security:
        - bearer: []
components:
  schemas:
    ProxyDeviceListResponseDto:
      type: object
      properties:
        devices:
          description: List of proxy devices
          type: array
          items:
