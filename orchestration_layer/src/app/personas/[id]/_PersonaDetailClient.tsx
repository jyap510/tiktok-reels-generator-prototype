...
export default function PersonaDetailClient({
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      {/* Back link */}
      <Link href="/personas" className="inline-flex items-center gap-2 text-neutral-400 hover:text-white text-lg mb-6 transition-colors">
        <span>←</span>
      ...
      <div className="flex gap-6 items-start">
        <img
          src={persona.avatar}
          alt={persona.handle}
          className="w-[240px] h-[240px] rounded-xl object-cover bg-neutral-800 shrink-0"
        />
        <div className="flex-1 min-w-0 pt-1">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <h1 className="text-4xl font-bold text-white">@{persona.handle}</h1>
            <span className={`text-base px-2 py-0.5 rounded-full font-medium ${statusColor[persona.status] ?? "bg-neutral-800 text-neutral-400"}`}>
              {persona.status}
            </span>
          </div>
          <p className="text-lg text-neutral-200 mb-1">{persona.niche}</p>
          <p className="text-lg text-neutral-300 mb-1">{persona.followers} followers</p>
          <p className="text-lg text-neutral-200 mb-2">{persona.bio}</p>
          <p className="text-lg text-neutral-300 italic mb-6">{persona.voice}</p>
          ...
          {visibleRuns.map((run) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`px-2 py-0.5 rounded-full text-base font-medium shrink-0 ${runStatusColor[run.status]}`}>
                  {run.status}
                </span>
              ...
              {/* Frames row */}
              <div>
                <p className="text-base text-neutral-300 uppercase tracking-wider mb-3">Frames</p>
                <div className="flex gap-4">
                  {run.media.frames.map((frame, i) => (
                    <div key={i} className="flex flex-col items-center gap-1.5">
                      <p className="text-base text-neutral-300">{clipLabels[i]}</p>
                      {frame ? (
              ...
              {/* Clips row */}
              <div>
                <p className="text-base text-neutral-300 uppercase tracking-wider mb-3">Clips</p>
                <div className="flex gap-4">
                    ...
                    <div key={i} className="flex flex-col items-center gap-1.5">
                      ...
                      {clip ? (
                        <div className="w-[240px] aspect-[9/16] rounded-lg overflow-hidden bg-black">
                          <video src={clip} controls muted className="w-full h-full object-cover" />
                        </div>
                      ) : (
                        <div className="w-[240px] aspect-[9/16] rounded-lg bg-neutral-800 border border-neutral-700 flex items-center justify-center">
                          <span className="text-base text-neutral-500">Failed</span>
              ...
              {/* Final video */}
              <div>
                <p className="text-base text-neutral-300 uppercase tracking-wider mb-3">Final Video</p>
                <div className="w-[420px] aspect-[9/16] rounded-lg overflow-hidden bg-black">
                  <video src={run.media.final} controls muted className="w-full h-full object-cover" />
                </div>
              </div>

              {/* Bottom row: voiceover + upload button */}
              <div className="flex items-end justify-between gap-4">
                <p className="text-base text-neutral-200 italic line-clamp-2 flex-1 min-w-0">
                  &ldquo;{run.prompts.voiceover_script}&rdquo;
                </p>
                <button
                  onClick={() => alert("Queued: " + run.run_id)}
                  className="shrink-0 px-3 py-1.5 border border-neutral-700 text-neutral-200 text-base font-medium rounded-lg hover:border-neutral-500 hover:text-white transition-colors"
                >
                  Upload to TikTok
                </button>
              </div>
